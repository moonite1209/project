...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, segment, semantic, gt_alpha_mask,
                 image_name, uid,
    ...
    #     feature_map = torch.from_numpy(np.load(language_feature_name + '_f.npy')) # (n个分割区域, 3)
    ...
    def get_language_feature(self, feature_level):
        return torch.from_numpy(self.segment[feature_level]).cuda(), torch.from_numpy(self.semantic).cuda()
