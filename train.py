...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if (iteration - 1) == debug_from:
            pipe.debug = True
        render_pkg = render(viewpoint_cam, gaussians, pipe, background, opt.mode) #TODO 渲染
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, dataset, iteration, blending_semantic_loss, semantic_loss, rgb_loss, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background, opt.mode))
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    semantic_map=render_pkg["language_feature_3d"]
                    gt_semantic_map, mask=viewpoint.get_semantic(feature_level=dataset.feature_level)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[5_000, 10000, 15000, 20000, 25000, 30_000])
